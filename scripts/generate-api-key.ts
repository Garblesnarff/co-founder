...
const roles = process.argv[4]?.split(',') || ['admin'];

if (!name) {
  console.log('Usage: bun run scripts/generate-api-key.ts <name> [user_id] [roles]');
  console.log('Example: bun run scripts/generate-api-key.ts "Rob CLI" rob admin');
  process.exit(1);
}
...
async function generateKey() {
  ...
  try {
    // Generate key
    const rawKey = `cofounder_${crypto.randomBytes(24).toString('base64url')}`;
    ...
    console.log(JSON.stringify({
      mcpServers: {
        cofounder: {
          httpUrl: 'https://cofounder.infiniterealms.tech/mcp',
          headers: {
